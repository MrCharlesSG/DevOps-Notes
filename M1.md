...
## PosgreSQL and Djang
Log in to the workstation VM in RH academy.
...
Create a custom network with DNS enabled:
...
podman run -d --name postgres \
  ...
  postgres:latest
Create a Django web application container:
...
podman run -d --name django \
  ...
  django:latest
Verify if using the default network would work:
...
Create a default web page in the Django application:
...
Persist PostgreSQL data using bind mounts:
...
Recreate the PostgreSQL container using a volume:
